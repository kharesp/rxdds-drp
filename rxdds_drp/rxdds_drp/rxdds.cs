...
namespace RTI.RxDDS
{
  ...
  public static class ObservableExtensions
  {
    ...
    public static IObservable<T> FirstAction<T>(this IObservable<T> source,Action<T> firstAction,IScheduler scheduler)
    {
        bool actionCompleted = false;
        return Observable.Create<T>(observer =>
            {
                return source.Subscribe(d =>
                    {
                        if (!actionCompleted)
                        {
                            firstAction(d);                            
    ...
    public static IObservable<T> FirstAction<T>(this IObservable<T> source,Action<T> firstAction)
    {
        return FirstAction<T>(source, firstAction, Scheduler.Immediate);
    }
    
    
    public static IObservable<IGroupedObservable<TPartitionKey,IList<T>>> Partitioner<T,TKey,TPartitionKey>
        (this IObservable<T> source, ReadOnlyDictionary<TPartitionKey,List<TKey>> partition_keys,
        ReadOnlyDictionary<TKey,TPartitionKey> keys_partition,Func<T,TKey> keySelector,Func<TKey,T> dummyGen)
    {       
        IDictionary<TPartitionKey, PartitionerSubject<T, TKey, TPartitionKey>> dict =
            new Dictionary<TPartitionKey, PartitionerSubject<T, TKey, TPartitionKey>>();
        foreach(var mapping in partition_keys)
            dict.Add(mapping.Key, new PartitionerSubject<T, TKey, TPartitionKey>(mapping.Key,
                                    mapping.Value, keySelector, dummyGen));
        return Observable.Create<IGroupedObservable<TPartitionKey,IList<T>>>(observer =>
        {
            foreach (var mapping in partition_keys)
                observer.OnNext(dict[mapping.Key]);

                return source.Subscribe(d =>
                    {
                        
                        var key = keySelector(d);
                        if(keys_partition.ContainsKey(key))
                        {
                            var partitionKey = keys_partition[key];
                            
                            //if (dict.ContainsKey(partitionKey))
                            dict[partitionKey].OnNext(d);
                            /*else
                            {
                                var subset_list = partition_keys[partitionKey];
                                dict.Add(partitionKey, new PartitionerSubjectTrial<T, TKey, TPartitionKey>(partitionKey,
                                    subset_list, keySelector, dummyGen));

                                observer.OnNext(dict[partitionKey]);
                                dict[partitionKey].OnNext(d);
                            }*/
                        }
  ...
  public class PartitionerSubject<T,TKey,TPartitionKey>: IGroupedObservable<TPartitionKey,IList<T>>,IObserver<T>
  {
     
      
      public TPartitionKey Key
      {
          get { return mPartitionKey; }
      }
      
      private IList<TKey> mSubsetList;
      private Func<T,TKey> mKeySelector;
      
      private Func<TKey,T> mDummyGen;

      private Subject<IList<T>> sub;
      private Dictionary<TKey, T> mValues;     
      private TPartitionKey mPartitionKey;
      
      
      //private IScheduler scheduler=new EventLoopScheduler();
      
      public PartitionerSubject
          (TPartitionKey partitionKey,IList<TKey> subset_list,Func<T,TKey> keySelector,Func<TKey,T> dummyGen)
      {
          mPartitionKey = partitionKey;
          mSubsetList = subset_list;
          mKeySelector = keySelector;
          mDummyGen = dummyGen;
          sub = new Subject<IList<T>>();
          mValues = new Dictionary<TKey, T>();
      ...
      {
          var key = mKeySelector(data);             
          mValues[key] = data;
         // var list = mValues.Values.ToList();
                        
          //scheduler.Schedule(() => sub.OnNext(list));             
          sub.OnNext(mValues.Values.ToList());
      }
      public void OnError(Exception e)
      {
          //scheduler.Schedule(()=>sub.OnError(e));
          sub.OnError(e);
      }
      public void OnCompleted()
      {
...
/*
 * public static IObservable<IGroupedObservable<TKey,T>> DemuxTrial<T,TKey>
        (this IObservable<T> source, Func<T, TKey> keySelector, Func<TKey, T> dummyGen,IList<TKey> keys)
    {
        var dict = new Dictionary<TKey, DDSKeyedSubject<TKey,T>>();
        foreach (var key in keys)
            dict.Add(key, new DDSKeyedSubject<TKey, T>(key));
        ...
        return Observable.Create<IGroupedObservable<TKey,T>>(observer =>
            {
                foreach (var key in keys)
                {                   
                    observer.OnNext(dict[key]);
                }
                foreach (var key in keys)
                {
                    dict[key].OnNext(dummyGen(key));
                }
                
                return source.Subscribe(
                    data =>
                    {              
                        TKey data_key = keySelector(data);                        
                        dict[data_key].OnNext(data);                                                
                    },
                    observer.OnError,
                    () =>
                    {
                        foreach (var ele in dict)
                            ele.Value.OnCompleted();
                        observer.OnCompleted();
                    });                    
            });               
    }
    
    public static IObservable<IGroupedObservable<TKey,T>> subsetTrial<T,TKey>(this IObservable<IGroupedObservable<TKey,T>> source,IList<TKey> subset_list)
    {        
        return Observable.Create<IGroupedObservable<TKey, T>>(observer =>
            {
                return source.Subscribe(
                    strm => {
                        if (subset_list.Contains(strm.Key))
                            observer.OnNext(strm);
                    },
                    observer.OnError,
                    observer.OnCompleted);
            });
    }
    public static IObservable<IList<IGroupedObservable<TKey,T>>> subsetTrial2<T,TKey>(this IObservable<IGroupedObservable<TKey,T>> source,IList<TKey> subset_list)
    {
        IList<IGroupedObservable<TKey, T>> streamList = new List<IGroupedObservable<TKey, T>>();
        return Observable.Create<IList<IGroupedObservable<TKey, T>>>(observer =>
            {
                return source.Subscribe(strm =>
                    {
                        if(subset_list.Contains(strm.Key))
                        {
                            streamList.Add(strm);
                            if(streamList.Count == subset_list.Count)
                            {
                                observer.OnNext(streamList);
                            }                                
                        }
                    },
                    observer.OnError,
